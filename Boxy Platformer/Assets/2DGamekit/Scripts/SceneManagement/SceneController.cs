...
namespace Gamekit2D
{
    ...
    public class SceneController : MonoBehaviour
    {
        ...
        public static void TransitionAfterGameOver(string sceneName)
        {
            //PlayerCharacter.PlayerInstance.damageable.SetHealth(PlayerCharacter.PlayerInstance.damageable.startingHealth);
            Instance.StartCoroutine(Instance.TransitionAfterGameOverCoroutine(sceneName));
        ...
        protected IEnumerator Transition(string newSceneName, bool resetInputValues, SceneTransitionDestination.DestinationTag destinationTag, TransitionPoint.TransitionType transitionType = TransitionPoint.TransitionType.DifferentZone)
        {
            ...
            if (m_PlayerInput == null && activeSceneName != "StartupLoadingScreen")
                m_PlayerInput = FindObjectOfType<PlayerInput>();
            if (activeSceneName != "StartupLoadingScreen")
        ...
        protected IEnumerator TransitionAfterGameOverCoroutine(string newSceneName)
        {
            m_Transitioning = true;

            GameObject.Find("Ellen").GetComponent<PlayerCharacter>().maxSpeed = 0f;
            ...
            yield return StartCoroutine(ScreenFader.FadeGameOverIn());

            yield return SceneManager.LoadSceneAsync(newSceneName);

            yield return StartCoroutine(ScreenFader.FadeLoadingIn());


            m_Transitioning = false;

            //ScreenFader.Instance.gameOverCanvasGroup.gameObject.SetActive (false);
        }
