...
namespace Gamekit2D
{
    public class HealthUI : MonoBehaviour
    {
        ...
        IEnumerator Start ()
        {
            ...
            scaleFactor = healthCanvas.scaleFactor;
            ...
            for (int i = 0; i < representedDamageable.startingHealth; i++)
            {
                ...
                //Check if localscale is less than the scale factor. If that's the case, then set the local scale to the new scale factor
                healthIconRect.localScale = new Vector2(healthIconRect.localScale.x < scaleFactor ? scaleFactor : healthIconRect.localScale.x
                    , healthIconRect.localScale.y < scaleFactor ? scaleFactor : healthIconRect.localScale.y);
