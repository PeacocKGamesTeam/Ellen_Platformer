...
using UnityStandardAssets.CrossPlatformInput; //kk added change
...
namespace Gamekit2D
{
    ...
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        void Start()
        {
			sceneController = GameObject.Find ("SceneController").GetComponent<SceneControllerWrapper> ();
        ...
        protected void UpdateCameraFollowTargetPosition()
        {
            ...
            //if (!Mathf.Approximately(PlayerInput.Instance.Vertical.Value, 0f)) kk comment change
            if (!Mathf.Approximately(CrossPlatformInputManager.GetAxis("Vertical"), 0f)) //kk added change
            {
            ...
            if (moveVertically)
            {
                //float desiredLocalPosY = PlayerInput.Instance.Vertical.Value * cameraVerticalInputOffset; //kk comment change
                float desiredLocalPosY = CrossPlatformInputManager.GetAxis("Vertical") * cameraVerticalInputOffset; //kk added change
        ...
        protected IEnumerator FlickerCustom()
        {
            float timer = 0f;

            while (timer < PowerUps.Instance.PowerUpDuration)
            {
                spriteRenderer.enabled = !spriteRenderer.enabled;
                yield return m_FlickeringWait;
                timer += flickeringDuration;
        ...
        public void UpdateFacing()
        {
            ...
            */
            ...
            //kk added change
            bool faceLeft = CrossPlatformInputManager.GetAxis("Horizontal") < 0f;
            bool faceRight = CrossPlatformInputManager.GetAxis("Horizontal") > 0f;
        ...
        public void GroundedHorizontalMovement(bool useInput, float speedScale = 1f)
        {
            ...
            */
            ...
            bool HorizontalReceivingInput;

            HorizontalValue = CrossPlatformInputManager.GetAxis("Horizontal");
            HorizontalReceivingInput = HorizontalValue == 0 ? false : true;
            
            float desiredSpeed = useInput ? HorizontalValue * maxSpeed * speedScale : 0f;
            float acceleration = useInput && HorizontalReceivingInput ? groundAcceleration : groundDeceleration;
        ...
        public void CheckForCrouching()
        {
            ...
            */
            //kk added change
            m_Animator.SetBool(m_HashCrouchingPara, CrossPlatformInputManager.GetAxis("Vertical") < -0.7f);
        }
        ...
        public void UpdateJump()
        {
            ...
            //if (!PlayerInput.Instance.Jump.Held && m_MoveVector.y > 0.0f) kk comment change
            if (!CrossPlatformInputManager.GetButton("Jump") && m_MoveVector.y > 0.0f) //kk added change
            {
        ...
        public void AirborneHorizontalMovement()
        {
            ...
            */
            ...
            float desiredSpeed = HorizontalValue * maxSpeed;

            float acceleration;

            //if (PlayerInput.Instance.Horizontal.ReceivingInput) kk comment change
            if (HorizontalReceivingInput) //kk added change
        ...
        public bool CheckForJumpInput()
        {
            ...
            //return PlayerInput.Instance.Jump.Down; kk comment change
            return CrossPlatformInputManager.GetButton("Jump"); //kk added change
        }

        public bool CheckForFallInput()
        {
            //return PlayerInput.Instance.Vertical.Value < -float.Epsilon && PlayerInput.Instance.Jump.Down;  kk comment change
            return CrossPlatformInputManager.GetAxis("Vertical") < -float.Epsilon && CrossPlatformInputManager.GetButton("Jump"); //kk added change
        }
        ...
        public void OnDie(bool shouldRespawn = true)
        {
            m_Animator.SetTrigger(m_HashDeadPara);

            //StartCoroutine(DieRespawnCoroutine(true, false, shouldRespawn));
            SceneController.Instance.GetComponent<SceneControllerWrapper>().TransitionAfterGameOver("StageSelection");
        }

        IEnumerator DieRespawnCoroutine(bool resetHealth, bool useCheckPoint, bool shouldRespawn = true)
        {
        ...
        public void StartFlickeringCustom()
        {
            m_FlickerCoroutine = StartCoroutine(FlickerCustom());
        }
        ...
        public bool CheckForMeleeAttackInput()
        {
            ...
            return CrossPlatformInputManager.GetButton("MeleeAttack"); //kk added change
        }
